...
var Item = exports.Item = function () {
  ...
  _createClass(Item, [{
    ...
    value: function removeTag(tag) {
      var before = this.tags.length;
      this.tags = this.tags.filter(function (cur) {
        return tag !== cur;
      });
...
var VertexTree = exports.VertexTree = function () {
  ...
  _createClass(VertexTree, [{
    ...
    value: function removeEdge(remove) {
      ...
      return remove.vertices().reduce(function (removed, v) {
        var didRemove = false;
        var item = _this2.at(v);
        if (item) didRemove = item.removeEdge(remove);
        if (didRemove && item.edges.length === 0) {
          _this2.remove(item.vertex);
        }
